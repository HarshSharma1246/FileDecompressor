...
public class HuffCompression {
    ...
    public static ByteNode createHuffmanTree(MinPriorityQueue<ByteNode> nodes) {
        while (nodes.len() > 1) {
    ...
    public static void main(String[] args) {
        compress(null, null); // Paste the Address of ("Input text File(Original Doc)", "Output text
                              // File(Compressed)")
        // New output file will be automatically at give address with tha name provided
        // in the address
        decompress(null, null);// Same as above paste the address ("Compressed file (by compress method)",
                               // "Output text file destination(will give the original file)
        // File address can be copied by going into the properties of that text file
        // (first write "" then paste into it)
        // If this code shows some problem run with Intellij IDEA
    }
}
